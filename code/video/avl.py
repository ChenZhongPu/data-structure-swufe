...
class AVLInsertion(Scene):
    ...
    def construct(self):
        # Title
        title = Text("AVL Tree Insertions: 6 → 2 → 1", font_size=36)
        title.to_edge(UP)
        self.play(Write(title))

        # Step 1: Insert 6
        step1 = Text("Step 1: Insert 6", font_size=24)
        step1.next_to(title, DOWN)
        self.play(Write(step1))

        # Create first node (6)
        node_6_group = self.create_node("6", [0, 2, 0])
        self.play(Create(node_6_group))
        self.wait(1)
        ...
        edge_6_2 = Line(
            start=node_6_group[0].get_center() + DOWN * 0.5,
            end=node_2_group[0].get_center() + UP * 0.5,
        )

        self.play(Create(node_2_group), Create(edge_6_2))
        self.wait(1)

        # Step 3: Insert 1
        step3 = Text("Step 3: Insert 1", font_size=24)
        step3.move_to(step2.get_center())
        self.play(FadeOut(step2), Write(step3))

        # Add node 1
        node_1_group = self.create_node("1", [-2.5, -1, 0])
        edge_2_1 = Line(
            start=node_2_group[0].get_center() + DOWN * 0.5,
            end=node_1_group[0].get_center() + UP * 0.5,
        )

        self.play(Create(node_1_group), Create(edge_2_1))

        # Show imbalance
        imbalance_text = Text("Left subtree too heavy!", font_size=24, color=RED)
        imbalance_text.to_edge(RIGHT)
        self.play(Write(imbalance_text))
        self.wait(1)

        # Rotation notification
        rotation_text = Text("Performing right rotation", font_size=24, color=GREEN)
