...
\begin{tikzpicture}[level/.style={sibling distance = 6cm/#1,
    ...
    \draw [-stealth, line width=0.4mm, draw=blue](3,-2) -- (5,-2)node[midway,above,shape=rectangle,draw=none]{Case 2};
    
    \node (righttree) [right=of lefttree, xshift=6cm] {}
    child {node (2nb) [arn_a] {$B$}
        child {node (2na) [arn_b] {$A$}
            child{node {$\alpha$}}
            child{node {$\beta$}}
        }
        child {node (2nd) [arn_r] {$D$}
            child {node [arn_b] {$C$}
                child {node {$\gamma$}}
                child {node {$\sigma$}}
            }
