...
\begin{frame}
    \frametitle{Exercise}
Given the \alert{\texttt{head}} of a linked list, how to check whether it is empty or not?
...
\pause
\textbf{Question}: What is the time complexity of \texttt{search()}?
\end{frame}
...
\begin{frame}
   ...
   How to implement \texttt{contains(head, k)} based on \texttt{search(head, k)}? 
...
\begin{frame}[fragile]
 ...
 Given the \alert{\texttt{head}} of a linked list, return the size of the list.
...
\begin{columns}
    ...
    \end{enumerate}
    \column{.6\textwidth}<2->
    \textbf{Question}: Can we even simplify the code using \texttt{Node}'s constructor?
\end{columns}
